...
const handlerOfReset = async (
  ...
  try {
    ...
    // DEBUG:
    // LOG OUT current iModel state
    console.log("-----------------------------------");
    console.log("iModel state just invoked handlerOfReset");
    const lks = await state.get();
    console.log(lks);
    console.log("-----------------------------------");
